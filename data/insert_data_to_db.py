...
try:
    ...
    if db.is_connected():
        ...
        sql = "CREATE TABLE orders ( id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY, number VARCHAR(255) NOT NULL UNIQUE, attractionId BIGINT NOT NULL, userId BIGINT NOT NULL, date DATE NOT NULL, time VARCHAR(50) NOT NULL, price INT NOT NULL, name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, phone VARCHAR(255) NOT NULL, status TINYINT NOT NULL, FOREIGN KEY(userId) REFERENCES user(id), FOREIGN KEY(attractionId) REFERENCES attractions(id));"
        cursor.execute(sql)
        db.commit()
